...
void Game::makeMove(int i, int j, int sgn_1, int sgn_2, int k, int mode, int observe){
	...
	if (mode==1){
		...
		if(sgn_1==0){
			...
			double xp=0;
			if(i+1<BOARDSZ_1)
				xp=board[player](i+1,j);
			if(i-1>=0)
				xm=board[player](i-1,j);
			x0=board[player](i,j);
			val=((xm+xp)/2)+x0;
			if(i+1<BOARDSZ_1)
				dist_neg(i+1,j)=xp/(xp+xm+2*x0);
			if(i-1>=0)
				dist_neg(i-1,j)=xm/(xp+xm+2*x0);
			dist_neg(i,j)=(2*x0)/(xp+xm+2*x0);
			...
			for(int z=i-1;z<=i+1;z++){
				...
				if(z==i)
					g=6.;
				for(int l=0;l<=2*k;l++){
			if(z+(l*sgn_1)<BOARDSZ_1 && j+(l*sgn_2)<BOARDSZ_2 && z+(l*sgn_1)>=0 && j+(l*sgn_2)>=0)
				dist_pos(z+(l*sgn_1),j+(l*sgn_2))=violent(k,l)*g;
				}
			}
			dist_pos.norm();
			...
			double xp=0;
			if(j+1<BOARDSZ_2)
				xp=board[player](i,j+1);
			if(j-1>=0)
				xm=board[player](i,j-1);
			x0=board[player](i,j);
			val=((xp+xm)/2)+x0;
			if(j+1<BOARDSZ_2)
				dist_neg(i,j+1)=xp/(xp+xm+2*x0);
			if(j-1>=0)
				dist_neg(i,j-1)=xm/(xp+xm+2*x0);
			dist_neg(i,j)=(2*x0)/(xp+xm+2*x0);
			...
			for(int z=j-1;z<=j+1;z++){
				...
				if(z==j)
					g=6.;
				for(int l=0;l<=2*k;l++){
			if(i+(l*sgn_1)<BOARDSZ_1 && z+(l*sgn_2)<BOARDSZ_2 && i+(l*sgn_1)>=0 && z+(l*sgn_2)>=0)
				dist_pos(i+(l*sgn_1),z+(l*sgn_2))=violent(k,l)*g;
				}
			}
			dist_pos.norm();
		}else{
